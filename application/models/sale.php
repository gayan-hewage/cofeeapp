...
class Sale extends CI_Model {
    ...
    function save($items, $customer_id, $employee_id, $comment, $payments, $sale_id = false) {
        ...
        $sales_data = array(
            ...
            'comment' => $comment,
            'shop_mode' => $this->sale_lib->get_store_type()
        ...
        foreach ($items as $line => $item) {
            ...
            if($item['overide']){
               $sale_price =  $item['price'] - $item['itemtax'];

            }

            $cur_item_info = $this->Item->get_info($item['item_id']);
            $store_quantity = $this->Item->getItemQuantity($item['item_id']);
            $sales_items_data = array
                ...
                'mode' => $this->sale_lib->get_store_type(),
                'overide'=> ($item['overide']) ? $item['overide'] : 0,
            ...
            $item_data = array(
                'quantity' => $store_quantity - $item['quantity'],
                'store_id' => $this->session->userdata('shop_id'),
                'item' => $item['item_id']
            );
            //$this->Item->save($item_data,$item['item_id']);
            $this->Item->saveItemQuantity($item_data, $item['item_id']);
    ...
    public function create_sales_items_temp_table() {
        $this->db->query("DROP TABLE IF EXISTS ospos_sales_items_temp");
        $SQL = "CREATE TEMPORARY TABLE " . $this->db->dbprefix('sales_items_temp') . "
        (SELECT date(sale_time) as sale_date, sale_time, " . $this->db->dbprefix('sales_items') . ".sale_id, " . $this->db->dbprefix('sales_items') . ".shop_id, comment,payments.payment_type, customer_id, employee_id, 
        " . $this->db->dbprefix('items') . ".item_id, supplier_id, item_unit_price, item_sale_price , item_tax_amount, quantity_purchased, item_cost_price, SUM(percent) as item_tax_percent,
        
        discount_percent, (item_sale_price*quantity_purchased-item_sale_price*quantity_purchased*discount_percent/100) as subtotal,

        " . $this->db->dbprefix('sales_items') . ".line as line, serialnumber, " . $this->db->dbprefix('sales_items') . ".description as description,


        ROUND((item_sale_price*quantity_purchased-item_sale_price*quantity_purchased*discount_percent/100)*(1+(SUM(percent)/100)),2) as total,


        ROUND((item_sale_price*quantity_purchased-item_sale_price*quantity_purchased*discount_percent/100)*(SUM(percent)/100),2) as tax,



        (item_sale_price*quantity_purchased-item_sale_price*quantity_purchased*discount_percent/100) - (item_cost_price*quantity_purchased) as profit
